...
@Composable
fun VerifyForgotPasswordByOtpContent(
    snackbarHostState: SnackbarHostState,
    loading: Boolean,
    state: VerifyForgotPasswordByOtpState,
    event: (VerifyForgotPasswordByOtpEvent)-> Unit) {
        ...
        Box(Modifier.fillMaxSize().padding(it)
            ...
            Column{
                ...
                OtpField(
                    ...
                    onValueChange = {
                        event(VerifyForgotPasswordByOtpEvent.TypeOtpEvent(it))
                    }
                )
                Spacer(modifier = Modifier.height(16.dp))
                if(remainingTime > 0){
                    ...
                    Button(
                        ...
                        onClick = {
                            event(VerifyForgotPasswordByOtpEvent.RequestOtpEvent)
                        }) {
...
fun EmailVerificationOtpContentPreview() {
    AuthenticationTheme(darkTheme = true) {
        ...
        val state = VerifyForgotPasswordByOtpState(
            otp = "32131",
